...
//        setSupportActionBar(mToolbar);
    mToolbar.setTitle(" All Teachers   ");
    init();
  ...
  private void init() {
    try {
      FirebaseDatabase.getInstance().getReference()
          .child("Teacher").addListenerForSingleValueEvent
          (new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
              ...
              for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                ...
                Log.i("DATABASE", teacher.getTname() + "Rollnoll" + teacher.getTid() +
                    "std_div" + teacher.getTclasses());
//                uid = students.getSid();
                String item;
                item = teacher.getTid()+ ": " + teacher.getTclasses();
                listisnull.add(item);
//                  listisnull.add(students.getSid() + "  " + students.getSname());
                Log.i("DATABASE item", item);

                if (!listisnull.isEmpty()) {
                  ArrayAdapter<String> adapter = new ArrayAdapter<String>(SearchTeacherActivity.this, android.R.layout.simple_list_item_1, listisnull);
                  listView.setAdapter(adapter);
                  Log.i("SEt", "data Successfully");
                } else {
                  Toast.makeText(getApplicationContext(),"No Teachers " ,Toast.LENGTH_LONG).show();
    ...
    listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
      ...
      public void onItemClick(AdapterView<?> parent, View view, int position,
                              long id) {
        Intent intent = new Intent(getApplicationContext(), ViewFacActivity.class);
